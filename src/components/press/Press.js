...
const Press = () => {
    const { newsData, isSubscribed } = useNewsData(18);
    const { hoveredPressOrders, setHoveredPressOrder, handleSubscribe } = usePress();

    return (
        <GridContainer>
            {
                newsData.map(({ logoImgUrl, id:pressId }, order) => {
                    return (
                        <GridItem key={order} onMouseOver={setHoveredPressOrder.bind(null, order)} onMouseOut={setHoveredPressOrder.bind(null, null)}>
                            {
                                hoveredPressOrders !== order ? <LogoImage logoUrl={ logoImgUrl }/> : <HoveredPress isSubscribed={(isSubscribed(pressId))} handleSubscribe={handleSubscribe.bind(null, pressId)}/>
                            }
                        </GridItem>
                    );
                })
